...
import { z } from 'zod';
import { calculateOrderTotalCents, Order } from '../domain/order.js';
...
export function ordersRouter(): Router {
  ...
  router.post('/orders', (req, res) => {
    ...
    const totalPriceCents = calculateOrderTotalCents(
      items.map((it, idx) => ({ id: `${id}-${idx}`, size: it.size, toppings: it.toppings }))
    );
    const order: Order = { id, items: items.map((it, idx) => ({ id: `${id}-${idx}`, ...it })), address, status: 'created', totalPriceCents };
    orders.set(id, order);
